...
public class TestBase {
    ...
    public void veifyGroupListInUI() {
    ...
    public void veifyContactListInUI() {
        if (Boolean.getBoolean("verifyUI")) {
            Contacts dbContacts = app.db().contacts();
            Contacts uiGroups = app.contact().all();
            assertThat(uiGroups, equalTo(dbContacts.stream().map((c) -> new ContactData().withFirstName(c.getFirstName()).withLastName(c.getLastName())
                    .withHomePhone(c.getHomePhone()).withMobilePhone(c.getMobilePhone()).withWorkPhone(c.getWorkPhone()).withAllPhones(c.getAllPhones())
                    .withAllEmails(c.getAllEmails())).collect(Collectors.toSet())));
        }
    }
